<template>
  <div class="w-100 p-2 content">
    ...
    <!-- <button
      class="button is-success noprint"
      v-if="admin && !individual"
      @click="exportReport"
      formtarget="_blank"
    >
      <i class="fas fa-file-excel"></i>
    </button> -->
    <div class="my-2" v-for="(evalg, index) in grades" :key="index">
      <h1><i class="mr-2" :class="evalg.icon"></i> {{ evalg.namegroup }}</h1>
      <table class="grades has-background-light mt-3" id="evaluationReport">
        <th>{{ trans.get("evaluation.student") }}</th>
        <th
          v-for="(tag, index) in evalg.evaluation[0].grades"
          :key="index + '-' + tag.id"
        >
          {{ tag.name }}
          <br />
        ...
        <th>Total</th>
        <tbody>
          <tr v-for="student in evalg.evaluation" :key="student.student_id">
            <td>{{ student.name }}</td>
            <td v-for="grade in student.grades" :key="grade.id">
              <span
                v-if="
                  admin || (settings.eval_type != 1 && settings.eval_type != 2)
                "
                ><span
                  :class="getGradeClass(getGrade(grade))"
                  class="tag is-size-6"
                  >{{ getGrade(grade) }}</span
                ></span
              >
              <span v-if="settings.eval_type == 1">
                <i
                  :class="'fas ' + getEmoji(getGrade(grade)) + ' rateEmoji'"
                ></i>
              </span>
              <span
            ...
            <td>
              ...
              >
                <span
                  :class="getGradeClass(finalGrade(student))"
                  class="tag is-size-6"
                  >{{ finalGrade(student) }} / {{ settings.eval_max }}</span
                >
              </span>
              <span v-if="settings.eval_type == 1">
                <i
                  :class="'fas ' + getEmoji(finalGrade(student)) + ' rateEmoji'"
                ></i>
              </span>
              <span
                v-if="settings.eval_type == 2"
...
export default {
  ...
  methods: {
    ...
    exportReport: function () {
      ...
      axios
        .post(
          "/classroom/evaluation/exportEvaluationReport/",
          {
            headings: headings,
            values: elements,
          },
