<template>
  <div>
    <button class="button is-primary has-margin-y-3" @click="showAddModal">{{ trans.get('events.add') }}</button>
    <div class="has-margin-0 w-100">
      ...
      >
        <template slot-scope="props">
          <b-table-column field="name" :label="trans.get('events.title')" sortable><span v-html="trans.get(props.row.title)"></span></b-table-column>

          <b-table-column field="url" :label="trans.get('events.content')" sortable><span v-html="trans.get(props.row.content)"></span></b-table-column>
          ...
          >
            <a
              v-tippy
              :content="isDisabled(props.row.id) ? trans.get('general.enable') : trans.get('general.disable')"
              @click.prevent="toggle(props.row.id)"
              class="button  is-small has-margin-right-3"
              :class="{'is-dark': !isDisabled(props.row.id), 'is-light': isDisabled(props.row.id)}"
            >
    ...
    <b-modal :active.sync="isModalActive" has-modal-card full-screen :can-cancel="false">
      <div class="modal-card" style="width: auto">
        ...
        <section class="modal-card-body is-relative" id="confetti-bg">
            ...
            <button class="button is-primary" type="button" @click="addEvent(0)" v-if="!edit">{{ trans.get('events.add') }}</button>
            <button class="button is-info" type="button" @click="addEvent(1)" v-else>{{ trans.get('events.edit') }}</button>
        </section>
...
export default {
  ...
  methods: {
    addEvent(type) {
      if(type == 0) {
        axios.post('/classroom/events/add', {event: this.event})
          .then(response => {
            location.reload();
          });
      } else  {
        axios.post('/classroom/events/edit', {event: this.event})
          .then(response => {
            location.reload();
          });
      }
    ...
    toggle(event) {
      axios.post('/classroom/' + this.classroom.code + '/events/disable', {id: event})
        .then(response => {
