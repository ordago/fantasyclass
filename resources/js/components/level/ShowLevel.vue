            ...
            <input :id="'file' + levelJson.id" type="file" style="display: none" @change="getImage">
                ...
                <div class="columns w-100 is-variable is-0 has-padding-y-2">
                    ...
                    <figure class="image is-128x128">
                        <label class="cursor-pointer" :for="'file' + levelJson.id"><img style="width: 128px; height: 128px" :src="image" class="has-padding-2 card-shadow-s is-full-rounded"></label>
                    </figure>
                    </div>
                    <div class="column card has-padding-4 rounded-right card-shadow-s">
                        <p class="is-size-4">Level {{ levelJson.number }}</p>
                        <p class="has-margin-y-2"><i class="fas fa-fist-raised colored"></i> Needed experience</p>
                        ...
                        <h1 class="is-2"><textarea style="resize: none;" class="input" v-model="levelJson.description"></textarea></h1>
                        <div class="has-text-right has-margin-top-2">
                            <button class="button is-dark" @click="update"><i class="fas fa-edit"></i></button>
                            <button class="button is-danger" @click="remove" v-if="last"><i class="fas fa-trash-alt"></i></button>
                        </div>
                    </div>
                </div>
    </div>
...
<script>
  export default {
        ...
        methods: {
         ...
         update: function() {
              ...
              const config = {
                ...
                formData.append('file', document.getElementById('file' + this.levelJson.id).files[0])
                ...
                axios.post('/classroom/'+ this.code + '/levels/' + this.levelJson.id, formData, config)
                .then(response => {
        ...
        remove: function() {
                axios.delete('/classroom/level/' + this.levelJson.id)
                            .then(response => {
                                //   if(response.data === 1) {
