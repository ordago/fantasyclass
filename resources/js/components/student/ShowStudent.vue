                ...
                <div class="columns is-multiline is-variable" v-if="admin">
                    <div v-for="item in items" class="column has-padding-y-2 is-6-tablet is-12-mobile is-4-desktop is-3-fullhd" v-bind:key="item.id">
                            <b-field>
                                ...
                                <b-numberinput @input="updateInventory(item.id, item.count)" min="0" icon-pack="fa" v-model="item.count" expanded controlsPosition="compact"/>
                            </b-field>
                    </div>
                </div>
                        ...
                        <div class="inventory" id="inventory" v-if="!admin">
                            ...
                            <div>
                                <div v-for="item in student.items" class="inventory_item" v-bind:key="item.id">
                                            <img :src="item.icon" class="item">
                                            <div class="number_items">{{ item.pivot.count }}</div>
                                </div>
                            </div>
                            <div>
                                <div v-for="index in inventoryRemaining" class="inventory_item" v-bind:key="index">

                                </div>
                            </div>
                            <div>
                                <div v-for="gear in student.equipment" v-bind:key="gear.id" class="inventory_item inv_item_armor" v-bind:class="{ inv_item_armor_bronce: gear.offset == 1, inv_item_armor_silver: gear.offset == 2, inv_item_armor_gold: gear.offset == 3 }">
  ...
  export default {
        ...
        methods: {
            ...
            updateInventory(item, count) {
                axios.patch('/classroom/' + this.classroom.code + '/student/inventory', { 'id' : this.student.id , 'item_id': item, 'count' : count, '_method' : 'patch'  });
            }
